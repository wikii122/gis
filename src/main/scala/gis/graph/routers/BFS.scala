...
class BFS[A](g: Graph[A]) extends Router[A](g) {
  ...
  final override protected[this] def findRoutes(vertices: List[graph.Vertex], jumps: Int,
    ...
    case vertex :: vertexList => if (routes(vertex.name).vertices.length < jumps) {
        ...
        val updatedRoutes = analyzed.foldLeft(routes){(nameAll, edge) => val route = nameAll(vertex.name)
          routes + (edge.opposite(vertex.name).name -> new Route(vertex :: route.vertices, route.distance + edge.cost))}
